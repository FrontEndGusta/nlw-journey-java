...
public class TripController {
    ...
    public ResponseEntity<TripCreateResponse> createTrip(@RequestBody TripRequestPayload payload) {
        ...
        LocalDateTime startDate = LocalDateTime.parse(payload.starts_at(), formatter);
        LocalDateTime endDate = LocalDateTime.parse(payload.ends_at(), formatter);
    ...
    public ResponseEntity<ActivityResponse> registerActivity(@PathVariable UUID id, @RequestBody ActivityRequestPayload payload) {
        DateTimeFormatter formatter = DateTimeFormatter.ISO_DATE_TIME;
        LocalDateTime occurs_at = LocalDateTime.parse(payload.occurs_at(), formatter);

        Optional<Trip> trip = this.repository.findById(id);

        if(trip.isPresent()){
            Trip rawTrip = trip.get();
            LocalDateTime tripStart = trip.get().getStartsAt();
            LocalDateTime tripEnd = trip.get().getEndsAt();
            if (occurs_at.isBefore(tripStart) || occurs_at.isAfter(tripEnd)) {
                throw new ActivityOutsidePeriodException();
            }
